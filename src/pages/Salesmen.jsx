...
const Salesmen = () => {
    ...
    const filteredSalesmen = salesmen.filter(salesman => {
        return salesman.SalesmanName.toLowerCase().includes(searchTerm.toLowerCase()) ||
                ...
                <tbody className="bg-white">
                    {filteredSalesmen.length > 0 ? (
                        filteredSalesmen.map(salesman => (
                            <tr key={salesman.SalesmanID} className="hover:bg-gray-100 transition-all duration-200">
                                <td className="py-4 px-4 border-b border-gray-300 text-gray-800">{salesman.SalesmanID}</td>
                                <td className="py-4 px-4 border-b border-gray-300 text-gray-800">{salesman.SalesmanName}</td>
                                <td className="py-4 px-4 border-b border-gray-300 text-gray-800">{salesman.ContactNumber}</td>
                                <td className="py-4 px-4 border-b border-gray-300 text-gray-800">{salesman.Email}</td>
                                <td className="py-4 px-4 border-b border-gray-300 text-gray-800">{salesman.Department}</td>
                                <td className="py-4 px-4 border-b border-gray-300 text-gray-800">{new Date(salesman.HireDate).toLocaleDateString()}</td>
                                <td className="py-4 px-4 border-b border-gray-300 text-gray-800">{salesman.CommissionRate}%</td>
                            </tr>
                        ))
                    ) : (
                        <tr>
                            <td colSpan="7" className="text-center py-4 text-gray-800">No salesmen found</td>
                        </tr>
                    )}
                </tbody>
            </table>
