            ...
            <table className="shadow-md min-w-full border-collapse rounded-lg overflow-hidden mt-4">
                ...
                <tbody className="bg-white">
                    {orders.map((order, index) => (
                        <tr key={`${order.OrderID}-${index}`} className="hover:bg-gray-100 transition-all duration-200">
                            <td className="py-4 px-4 border-b border-gray-300 text-gray-800">{index + 1}</td>
                            <td className="py-4 px-4 border-b border-gray-300 text-gray-800">{order.CustomerName}</td>
                            <td className="py-4 px-4 border-b border-gray-300 text-gray-800">{order.ContactNumber}</td>
                            <td className="py-4 px-4 border-b border-gray-300 text-gray-800">{order.Email}</td>
                            <td className="py-4 px-4 border-b border-gray-300 text-gray-800">
                                {new Date(order.OrderDate).toLocaleDateString()}
                            </td>
                            <td className="py-4 px-4 border-b border-gray-300 text-gray-800">{order.OrderStatus}</td>
                            <td className="py-4 px-4 border-b border-gray-300 text-gray-800">{order.Quantity}</td>
                            <td className="py-4 px-4 border-b border-gray-300 text-blue-600">
                                <button onClick={() => handleSelected(order.CustomerName)} className="text-blue-500 underline">View Invoice</button>
                            </td>
